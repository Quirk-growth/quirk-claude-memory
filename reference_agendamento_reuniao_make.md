...
**NOVO CENÁRIO do CRM (substitui o ClickUp pro funil comercial): 4903930 "Agendamento Comercial (CRM)"** (criado 03/09). CRM `acao: 'agendar'` → `dispararAgendamento()` → env `COMERCIAL_AGENDA_WEBHOOK_URL` (só no Render) → webhook 2813314 → Router por `responsavelEmail` → `createAnEvent` (Renan conn 4832988 cal contato@ + Zoom fixo | Yuri conn 4000944 cal contato.yurivitoria@ + Meet) → HTTP `POST /api/crm/agendamento-retorno` grava `meetingUrl` no card. O 3704159 continua rodando, mas só pra ClickUp.

**GOTCHA (15/09/2026): o Yuri tem DOIS e-mails.** Login na área de membros = `yuri.snv@gmail.com`; agenda Google = `contato.yurivitoria@gmail.com`. O CRM manda o e-mail do USUÁRIO vinculado ao vendedor (login); o filtro da rota do Yuri só aceitava o da agenda → nenhuma rota casava → evento não criado, lead não convidado, execução VERDE. Corrigido adicionando no filtro (OU): os dois e-mails + nome exato "Yuri Vitória" (o fallback por nome estava na spec e nunca foi implementado). Verificado ponta a ponta com o lead 504 (Ariel): Meet voltou pro card.

**How to apply — diagnóstico rápido:** a assinatura é a contagem de operações das execuções do 4903930: **3 = rota rodou; 1 = só o webhook, nenhuma rota casou.** O detalhe da execução NÃO guarda o bundle recebido. Pra saber o que o CRM manda: `crm_vendedores.usuario_id → users.email`. Pra saber a conta real da conexão Google: RPC `listCalendars` (acima). Todo closer novo precisa de rota nova E de conferir se o e-mail de login dele bate com o da agenda.
