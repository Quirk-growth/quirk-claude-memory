...
metadata: 
  ...
  modified: 2026-08-18T16:25:09.652Z
---
...
2. **`PUBLICAR_POSTS_SECRET` no Render + Render Cron Job** (`*/5 * * * *`) — confirmado FALTANDO em prod (fingerprint do deploy: `POST /api/cron/publicar-posts` responde 500 "não configurado"). Doc pronta em `docs/operacao/render-cron-publicar-posts.md`.
