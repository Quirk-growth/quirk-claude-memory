...
metadata: 
  ...
  modified: 2026-09-26T20:13:44.676Z
---
...
**Fase LP (26/09/2026):** VSL gravada e editada ("VSL Quirk_Ver1.mp4" no Drive, 16min10s, 12,2GB 4K60 → transcodificada local p/ web: 1080p30 ~286MB + 720p30, h264_videotoolbox). LP construída em `~/quirk-lps/vsl/index.html` (monorepo LPs-quirk; mp4 no .gitignore): identidade Growth tech dark, player custom SEM controles (smart autoplay mudo + overlay "ativar som", barra de progresso falsa `x^0.5` — rápida no início, converge no fim, sem seek), reveals por currentTime persistidos em localStorage `quirk_vsl_v1`: botão Sala de Guerra aos 180s, formulário aos 300s. Pixel Quirk 905158130958739 c/ eventos VideoStart/Video3min/Video5min/Lead(eventID). Form multi-step 3 passos → webhook n8n `POST /webhook/lp-vsl-lead` (workflow **uuKWujYX3kRKiDsM** "Leads LP VSL [Comercial] → CRM", ATIVO e testado ponta a ponta): normaliza → POST intake CRM `membros.quirkgrowth.com.br/api/crm-leads/entrada/4Xk7F8oRAJLcZb5SUv9PkU9lKdhX_XQq` (mesmo token da LP Bio/Make 4896527; campanha="VSL"; respostas[] carrega empresa/instagram/VGV/nicho/investimento/UTMs) → Gmail interno (por ora SÓ contato@; add Yuri+Rodrigo no go-live). Preview local via launch.json "lp-vsl" porta 8098. GOTCHA: `python http.server` não suporta Range → seek de vídeo não funciona em preview local (não é bug da página). **Fase Vimeo (26/09):** Renan pediu o vídeo no Vimeo (banda do cPanel) — player da LP reescrito pra iframe + Player API (SDK player.vimeo.com/api/player.js, `pointer-events:none` no iframe, overlays/barra/reveals próprios via `timeupdate`; barra falsa e reveals idênticos). Token do Vimeo fica em **`~/ig-auto-ads/.vimeo_token`** (NÃO nos .env; conta "Renan Real", Starter — aceita privacy.view='disable' + embed whitelist + download off). Vídeo da VSL: **id 1230560399**, domínios liberados quirkgrowth.com.br + localhost (localhost permite preview local). Ajustes de copy: sem travessões; **formulário abre JUNTO com a Sala de Guerra aos 180s** (evento pixel Video5min continua marcando 300s, desacoplado do reveal); link real do grupo: chat.whatsapp.com/G7sf7pEjTuY3zVsqqAG0A6. GOTCHA: id de vídeo demo 76979871 morreu (oEmbed 404) — pra testar player use 1084537. Com Vimeo, o pacote de publicação caiu pra ~160KB (index+logo+poster; mp4s locais são só arquivo).
**Pendências:** DNS Cloudflare (proposto vsl.quirkgrowth.com.br; aguardando API token do Renan), upload cPanel (acesso FTP/SSH ou ZIP manual), e-mails Yuri+Rodrigo no n8n no go-live, APAGAR lead "TESTE Claude — apagar" do CRM.
