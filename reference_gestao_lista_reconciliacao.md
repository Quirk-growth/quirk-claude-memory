...
metadata:
  ...
  modified: 2026-08-03T13:30:07.857Z
---
...
**Verba/status ao vivo (02/ago):** `init_gestao` também busca `daily_budget` dos conjuntos (`GET /act_X/adsets?fields=campaign_id,daily_budget`) e usa como verba exibida — reflete mudança feita via Auto Ads OU direto no Gerenciador na hora. `build_gestao_confirmation_msg` já confirma toda ação bem-sucedida (✓ pausada/reativada/encerrada/verba/público/geo).

⚠️ **Gotcha n8n que causou regressão grave:** numa **query de nó Postgres**, `{{ $('algum_no')... }}` é avaliado ANTES de rodar, **mesmo dentro de um `CASE WHEN` que não se aplica**. Se `algum_no` não rodou naquele caminho → erro `Node hasn't been executed` e a query TODA quebra. (Aconteceu: pus `$('build_targeting_atualizado')` no `update_db_campanha` p/ o público, e quebrou VERBA/PAUSAR/REATIVAR/ENCERRAR que não rodam esse nó.) **Regra: nunca referencie um nó condicional direto na query Postgres — resolva o valor num code node anterior (`prep_update_db`, com try/catch) e use `$json.campo`.**
