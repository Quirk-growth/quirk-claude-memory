...
- **`push` desligado em produção:** `payload.config.ts` → `push: process.env.NODE_ENV !== 'production'` (dev/test = ligado; prod = off). Boot limpo/rápido sobre o pooler. **Consequência: TODA coleção nova em prod exige `CREATE TABLE` manual (+ a coluna `<slug>_id` em `payload_locked_documents_rels`, com FK ON DELETE CASCADE + índice) ANTES do deploy ficar Live** — o push não cria mais nada em prod. Pool ajustável via env `DB_POOL_MAX` (default 20). **⚠️ REINCIDENTE (30/jul/2026):** ao criar a coleção `pagamentos` esqueci a coluna `pagamentos_id` no lock rels → a query de locks (roda no fetch do global do banner na home) quebrou com `errorMissingColumn` e **derrubou a área de membros**. Introspectar só a tabela nova NÃO basta — o DDL de coleção nova SEMPRE precisa do bloco do `payload_locked_documents_rels`. Fix aplicado em prod via `pg` (aditivo/idempotente) restaura na hora.
...
**GOTCHA da migração manual — collection NOVA (06/ago, sistema caiu):** ao adicionar uma collection nova em prod por DDL manual (push OFF), NÃO basta criar as tabelas `<collection>*`. O Payload, ao registrar a collection, TAMBÉM adiciona colunas FK na tabela interna compartilhada **`payload_locked_documents_rels`** (uma coluna `<collection>_id` por collection, + FK ON DELETE CASCADE + índice). Eu esqueci disso ao subir Tarefas → o admin, ao navegar, consulta `payload_locked_documents` (join com todos os rels) e estourou `column payload_locked_documents_rels.listas_tarefas_id does not exist` (42703) = tela "This page couldn't load / server error" em TODO o admin (latente desde o deploy da collection; só aparece quando alguém abre uma view que consulta locks). **Fix:** `ALTER TABLE payload_locked_documents_rels ADD COLUMN <collection>_id integer` + FK REFERENCES <collection>(id) ON DELETE CASCADE + índice, pra cada collection nova. **Método correto de extração do DDL:** no banco de TESTE, procurar TODAS as colunas novas em QUALQUER tabela (incl. `payload_*_rels`), não só as tabelas da própria collection — `SELECT table_name,column_name FROM information_schema.columns WHERE column_name LIKE '%<collection>_id%'`. (payload_preferences_rels NÃO precisou no caso das tarefas.)
