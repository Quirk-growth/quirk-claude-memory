...
metadata: 
  ...
  modified: 2026-09-14T19:32:45.602Z
---
...
**CORREÇÃO — v3 foi descartada (11/set):** a autoavaliação embutida fez o GPT "virar avaliador" (cuspia nota/relatório em vez de criar). Renan mandou REMOVER o avaliador do gerador. Estado final = **gerador PURO (v4/v5)**: cria a copy completa (H1+H2 escritas + recurso criativo de gancho + Gancho/Body/CTA em 3-5 variações), SEM rubrica/nota/selo. A "base do avaliador" fica só dissolvida como regras de escrita (gancho lidera pelo destaque, body direto, eixo coerente, CTA do repertório ampliado c/ vídeo, sem emoji na copy, não inventar dado→[PREENCHER]) + uma checagem final silenciosa sem números. **v5 (anti-ancoragem):** copys saíam parecidas demais com os exemplos do prompt (água de coco, bloco, trena, chave) — o modelo copiava o exemplo. Fix: exemplos viraram props PROIBIDOS de reutilizar; lentes de criatividade genericizadas (partir SEMPRE do briefing deste imóvel); regra "gere 3+ ideias e descarte a óbvia" + "se serviria pra qualquer lançamento, refaça". prompt-gerador-copys.md ~7,8k chars.

**v6 — elemento didático + anti-vício (11/set):** (a) formato do elemento de gancho virou 4 campos prescritivos (O que usar / Como gravar passo a passo / Quando entra / Por que funciona) — Renan queria mais didático pra quem grava. (b) Novo colapso de modo: o GPT repetia "segurar placa/cartão/etiqueta com o número escrito" em quase toda variação. Fix: regra "MOSTRE, não escreva" (número vive na fala/headline, não no prop), PROIBIDO vício de texto impresso (máx 1 variação), e as 8 lentes viraram "TIPOS DE RECURSO" com mandato de usar um DIFERENTE por variação (objeto real, ação, som, prova de terceiro genuína, cenário, trocadilho, demonstração, reação). Aprendizado recorrente: exemplo concreto no prompt → o modelo copia; e ele sempre acha um atalho preguiçoso, tem que proibir explicitamente. prompt-gerador-copys.md ~7,7k chars (teto do Custom GPT é 8k, fica sempre apertado).

**v7 — simplicidade de execução (14/set):** Renan alertou que quem grava (corretor + 1 pessoa com celular) não tem recurso nem visão pra elementos "surreais". Fix: separar criatividade (na IDEIA) de execução (tem que ser boba). Removido "algo que ninguém no mercado faz". Adicionado FILTRO Nº1 "SIMPLES DE GRAVAR" (take único de celular, sem edição/efeito/equipamento/atuação, só o que está à mão, "simplifique até um iniciante gravar de primeira"), mote "ideia esperta + execução boba", regra "gere 3+ e fique com a mais simples que ainda surpreende", e "simples de gravar com celular" entrou na checagem final. Tensão a lembrar: anti-vício (não repetir placa/texto) vs simplicidade (placa é fácil) — resolvido mantendo o veto a texto impresso mas exigindo que a alternativa seja igualmente fácil (objeto/gesto real). prompt em ~7,96k chars.
